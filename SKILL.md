---
name: stage-relay
description: 用於大型、多階段的開發任務，需要跨 Session 或 Agent 持續追蹤進度，並要求各階段通過自動測試與使用者驗收後才能完成。
---
...
只有目前階段進入 `結束` 後，才能正式開始下一個尚未完成的階段。將下一階段設為 `開發中`，依其目標完成開發與自動測試，測試通過後交由使用者驗收，驗收完成後再結束該階段。
...
驗收內容以 `task.md` 中該階段的驗收條件為主要依據。只有使用者明確表示驗收通過，才可將狀態改為 `使用者已驗收完成`。

如果使用者表示驗收失敗、發現 Bug 或要求修改：

1. 不得標記為 `使用者已驗收完成`。
2. 將階段恢復為 `開發中`。
3. 在 `task.md` 記錄使用者回報的問題。
4. 修正問題並重新完成開發。
5. 重新執行相關自動測試，不得沿用修改前的測試結果。
6. 測試全部通過後，再次交由使用者驗收。
